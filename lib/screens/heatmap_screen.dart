...
class _HeatmapScreenState extends State<HeatmapScreen> {
  ...
  double minValue = 0.0;
  double maxValue = 0.0;
  double geographicWidthRatio = 1.0;
  ...
  List<double>? _optimalRangeOverride;
  // Run selection state
  ...
  void initState() {
    super.initState();
    _loadData();
    // Default select all base metrics (exclude 'All')
    WidgetsBinding.instance.addPostFrameCallback((_) {
      try {
        final saved = context.read<AppSettings>().selectedMetrics;
        setState(() {
          final base = metrics.where((m) => m != 'All').toSet();
  ...
  void _updateGridAndValues() {
    ...
    final newGrid = heatmapData.grid;
  ...
  void _selectFarmAverage(int farmId) {
    ...
    setState(() {
      ...
      _selectedRunIndex = _farmAverageSentinel;
      startTime = farmStart;
      endTime = farmEnd;
    });
    _updateGridAndValues();
